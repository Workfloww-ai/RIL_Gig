    ...
    <SafeAreaView className="flex-1 bg-sand pt-8">
      <View className="bg-cream px-4 py-4 flex-row items-center border-b border-sage/10 shadow-sm z-10">
        ...
        <View className="flex-1">
          ...
          <Text className="font-bold text-slate text-center" numberOfLines={1}>
            {module.title}
          </Text>
        </View>
        <View className="w-10" />
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="flex-row mx-4 mt-4 bg-sage/10 rounded-full p-1">
          ...
          >
            <Text className={`font-semibold ${activeTab === 'video' ? 'text-moss' : 'text-sage'}`}>
              📹 Video Lesson
            </Text>
          </TouchableOpacity>
          <TouchableOpacity 
          ...
          >
            <Text className={`font-semibold ${activeTab === 'audio' ? 'text-moss' : 'text-sage'}`}>
              🎙 Podcast (Audio)
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="bg-cream mx-4 rounded-3xl overflow-hidden shadow-sm border border-sage/10 p-1 mb-6">
          ...
          <View className="p-5 opacity-100" style={{ opacity: player ? 1 : 0.5 }} pointerEvents={player ? 'auto' : 'none'}>
            ...
            <View className="flex-row justify-between items-center mb-5">
              <Text className="text-sage text-xs font-medium tracking-widest">
        ...
        <View className="bg-cream mx-4 rounded-3xl p-6 shadow-sm border border-sage/10 mb-10">
          <Text className="text-lg font-bold text-slate mb-2">✨ Module Summary</Text>
          <Text className="text-sage leading-relaxed mb-6">
          ...
          <View className="bg-sand p-4 rounded-2xl border border-sage/10">
            ...
            <View className="flex-row flex-wrap gap-2">
              {module.key_module_topics?.map((topic: string, i: number) => (
                <View key={i} className="bg-cream border border-sage/20 px-3 py-1.5 rounded-lg shadow-sm">
                  <Text className="text-sage text-xs font-medium">• {topic}</Text>
                </View>
              ))}
            </View>
          </View>
